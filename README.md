...
导入前分析内容哈希、来源、运行时名称和搜索相似度，区分完全重复、同来源、同名不同内容和候选相似项。遇到需要判断的情况交给用户选择，不静默覆盖。导入后按关系类别分组确认（完全重复、内容一致的复制副本、同名不同内容、通用目录直接读取、通用目录链接引用、识别未知），每组显示影响数量、受影响 Agent 与回退方式，未确认不放行；识别未知与未决冲突进入待办，不静默跳过。

### 关系转换可回退

复制副本与共享目录引用可以在确认影响后替换为集中库管理链接：先备份、再建新链接验证、最后移除原入口，同一位置不留双入口；失败时保留原入口或从恢复点回退，链接能力不足时明确告知而不是静默改为复制覆盖。Agent 详情页提供目录与关系矩阵（目录角色、识别状态、共享消费者），Skill 详情页提供来源存证、关系类型与移除影响预览；可选的 AI 冲突分析只给出短结论与建议，不替代确定性检查，也不改写用户裁决。

### 确定性安全检查，LLM 只是可选增强

基础检查不依赖大模型，识别危险命令、疑似敏感信息和提示词注入等风险。需要更深层语义分析时，可以单独启用 LLM 检查；它不会替代基础检查，也不会自动修改 Skill。

LLM 接入统一在一个设置入口：25 个内置供应商预设（按产品线区分，如 GLM 普通 API 与 Coding Plan 不复用端点）与自定义 Base URL，模型列表拉取失败可手动填写，连接测试分“服务可达 / 模型可调用 / 结构化输出兼容”三级展示，密钥保存在操作系统凭据存储。请求参数由“传输协议 × 供应商兼容档 × 结构化策略 × 推理策略”四层共同决定，不按名称或地址猜厂商。AI 安全检查、描述翻译、语义重复分析、联网搜索辅助四项能力默认关闭、逐项开关并说明发送范围，详见 [LLM 用户配置与隐私说明](docs/llm/用户配置与隐私说明-2026-09-13.md) 与 [LLM 供应商兼容矩阵](docs/llm/供应商兼容矩阵-2026-09-13.md)。
...
- 导入关系分组确认、复制副本/共享引用转管理链接、Agent 目录矩阵、Skill 详情关系视图、可选 AI 冲突分析及冲突组生产写入链路，以及来源副本治理与部署预览 pair 契约（2026-09-24 实施计划 15/15）已完成代码与自动化；真实设备验收待做。进度见 [开发状态](docs/development/开发状态-2026-09-24.md)、[自动化测试说明](docs/development/自动化测试说明-2026-09-24.md) 与 [验收矩阵](docs/development/功能完成度与验收状态矩阵-2026-09-24.md)
- 技能关系一级模块页面与统一异步执行反馈已进入收口：关系图谱、冲突工作台、治理清单具备实现和自动化证据；全部用户可触发的异步写入均已接入统一执行桥且失败进通知中心，Windows junction 与链接权限仍待真机取证。实施范围见 [实施计划](docs/superpowers/plans/2026-09-16-skill-relationship-module-and-unified-execution-implementation-plan.md)
...
- LLM API 密钥只保存在操作系统凭据存储；集中库、备份和标准导出只保留引用，不含密钥明文。
...
- [开发状态](docs/development/开发状态-2026-09-24.md) / [自动化测试说明](docs/development/自动化测试说明-2026-09-24.md) / [人工验收清单](docs/development/人工验收清单-2026-09-24.md) / [功能完成度与验收状态矩阵](docs/development/功能完成度与验收状态矩阵-2026-09-24.md)
