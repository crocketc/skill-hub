...
it("reports restore preparation failures in an alert region", async () => {
  const operations = baseOperations({
    pickDirectory: async () => "C:\\backup.skillhub",
    prepareRestore: async () => {
      throw { code: "backup.plan_failed", severity: "error", params: {}, actions: [] };
  ...
  expect(await screen.findByRole("alert")).toHaveTextContent(
    "操作失败（backup.plan_failed）。请稍后重试或重新打开页面。",
  );
});

it("reports restore commit failures in an alert region", async () => {
  const operations = baseOperations({
    pickDirectory: async () => "C:\\backup.skillhub",
    prepareRestore: async () => ({
      format_version: 1,
  ...
  expect(await screen.findByRole("alert")).toHaveTextContent(
    "操作失败（backup.commit_failed）。请稍后重试或重新打开页面。",
  );
});
