...
| 初始化批量导入引导 | 通过（自动化） | 扫描预览进入导入向导并预填首个来源；候选列表支持全选，冲突审阅与提交确认仍保留 |
| 扫描来源批量选择 | 通过（自动化） | 导入页保留全部扫描来源，可逐项勾选或全选；多个来源逐个获取候选并合并审阅 |
| 本地目录弹窗选择 | 待原生契约 | 当前仍保留手动路径输入；Tauri 原生目录选择器尚未接入，不能宣称已支持弹窗选目录 |
