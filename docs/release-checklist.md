...
| 前端测试 | 通过 | 当前 Windows 基线：72 个文件、403 项测试；macOS 最近基线已通过 |
...
| 集中库可见技能目录 | 通过（自动化） | 导入后的当前版本会物化到集中库 `skills` 目录；既有索引记录在启动时补齐缺失目录，内部对象与版本清单继续保留用于回滚 |
| 部署默认策略与手动覆盖 | 通过（自动化） | Agent 在当前账号支持链接时默认优先符号链接，否则安全降级复制；项目目标默认托管复制；部署页可显式覆盖为已支持的链接或复制方式 |
