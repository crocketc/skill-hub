...
| 前端测试 | 需在当前基线重跑 | 历史 Windows/macOS 结果不能直接作为 `1c937c2` 的当前证据 |
...
| 浏览器 E2E | 通过（自动化） | `pnpm test:e2e` 在 `eaee655` 基线上 22/22 通过：既有 10 条，加 `native-flow-preview` 12 条；运行 Vite 预览页并注入固定 IPC 返回值，只证明页面/交互和预览边界，不证明 Tauri 原生行为 |
