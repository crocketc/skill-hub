...
验收：对应 facade 测试、前端 native API 测试、TypeScript、Lint 和双平台本地 CI 通过。

### Task18.2 保存 Markdown 生成新版本（待开发）

- 明确文件选择/权限授予契约后，再实现 `save_skill_content`。
- 新版本必须通过 `SKILL.md` 基础校验，版本对象保持不可变，成功后切换当前版本。
- 覆盖取消、非法内容、写入失败和回滚测试。

### Task18.3 双平台验证与文档（待执行）

- Windows 与 macOS 均运行本地 CI 和 Task18 专项测试。
- 汇总测试结果，更新开发状态和 Task 报告。
