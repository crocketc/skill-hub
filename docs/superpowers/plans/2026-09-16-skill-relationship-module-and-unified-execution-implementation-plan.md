...
- 状态：**已确认，尚未开始编码**。本计划落实《技能关系模块与统一执行体验设计（2026-09-16）》，不替代其中的产品决策。
...
- 术语冻结：用户动作叫“纳入集中库管理”；技术细节可说明管理链接。冲突页面叫“冲突处理”，不是通用待办；治理页面叫“关系治理”。
...
- [ ] 关系事实投影有有限深度、版本/最后验证时间、无扫描读取、升级测试。
- [ ] 冲突处理只显示待确认 `uncertain`，AI 建议一定落到明确动作或治理预览。
- [ ] 关系治理按关系边工作，单/批量操作全程预览、确认、验证、回退可追溯。
- [ ] 图谱没有空画布，单中心深链稳定，搜索包含别名，筛选不生成多根图。
- [ ] 顶栏在途状态居中 360px、可分页查看，通知可清除但日志不可丢失。
- [ ] 概览保留既有图表和通用待办，新增关系内容不重复统计也不产生页面滚动条。
- [ ] 全部 Rust/TypeScript 契约绑定由生成器生成；全量自动化通过；Windows/macOS 人工验收记录已回填。
