...
- [ ] 全部 Rust/TypeScript 契约绑定由生成器生成；全量自动化通过；Windows/macOS 人工验收记录已回填。**已完成**：绑定生成器无漂移，完整 Rust 工作区测试与 `ci-local` 通过，前端单元 296 文件 / 1735 通过、E2E 387/387 通过。**仍需**：Windows/macOS 真机人工验收记录回填，当前发布阻塞项为《人工验收清单-2026-09-17》的 J-REL、RC 与 SR 项。
- [x] 用户可见动作词统一为“添加到 Agent/项目”“从 Agent/项目移除”，「部署」只作为名词短语（链接部署/复制部署/部署关系）与技术详情术语出现（Task 11；`terminology.test.ts` 防回归）。
- [ ] 关系测试夹具不依赖宿主符号链接能力：`facade_relationship_governance` 在缺少链接权限的账号上零失败。**已完成**：Task 12 的共享影响确认改为无链接编排夹具，核心语义有纯函数锚点，真实共享链接测试在创建夹具前显式守卫，自动化工作区测试通过。**仍需**：在无权限 Windows 账号真机复跑并回填 RC-15。
