...
- [ ] 关系测试夹具不依赖宿主符号链接能力：`facade_relationship_governance` 在缺少链接权限的账号上零失败。**已完成**：Task 12 的共享影响确认改为无链接编排夹具，核心语义有纯函数锚点，真实共享链接测试在创建夹具前显式守卫。**仍需**：在无权限 Windows 账号真机复跑并回填 RC-15。
