...
- 状态：**Task 1—12 的代码与自动化闭环已完成（2026-09-17）**；当前仅等待 Windows/macOS 人工验收记录。Task 11（用户可见动作词收敛）与 Task 12（关系测试夹具去链接依赖）是 2026-09-16 用户裁决后追加的非主线任务。本计划落实《技能关系模块与统一执行体验设计（2026-09-16）》，不替代其中的产品决策。当前交接入口见 `docs/development/` 四份 2026-09-17 文档。
...
- [x] 冲突处理只显示待确认 `uncertain`，AI 建议一定落到明确动作或治理预览；双侧对比、影响预览、异步执行和回写闭环已完成（Task 2、Task 7）。
- [x] 关系治理按关系边工作，单/批量操作全程预览、确认、验证、回退可追溯；治理页面、解除部署入口与顶栏反馈已完成（Task 3、Task 8、Task 4）。
- [x] 图谱没有空画布，单中心深链稳定，搜索包含别名，筛选不生成多根图；页面、路由、交互和窄视口自动化已完成（Task 1、Task 5、Task 6、Task 9）。
- [x] 顶栏在途状态居中 360px、可分页查看，通知可清除但日志不可丢失（Task 4/10；自动化与 E2E 证据见当前《自动化测试说明》）。
- [x] 概览保留既有图表和通用待办，新增关系内容不重复统计也不产生页面滚动条；关系缩略入口、冻结指标口径和窄视口自动化已完成（Task 9）。
- [ ] 全部 Rust/TypeScript 契约绑定由生成器生成；全量自动化通过；Windows/macOS 人工验收记录已回填。**已完成**：绑定生成器无漂移，完整 Rust 工作区测试与 `ci-local` 通过，前端单元 **165 文件** / 1735 用例通过、E2E 387/387 通过。**仍需**：Windows/macOS 真机人工验收记录回填，当前发布阻塞项为《人工验收清单-2026-09-17》的 J-REL、RC 与 SR 项；其中 RC-15/RC-16 与 SR-9 由 2026-09-17 独立复核新增。**勘误**：此前本行与《自动化测试说明》写的「296 文件」为误记，实际运行文件数为 165。
- [x] 用户可见动作词统一为“添加到 Agent/项目”“从 Agent/项目移除”，「部署」只作为名词短语（链接部署/复制部署/部署关系）与技术详情术语出现（Task 11；`terminology.test.ts` 防回归）。
- [ ] 关系治理链路在缺少链接权限的宿主上**可观测地跳过**、在有权限宿主上**真的跑过**。**已完成**：Task 12 的共享影响确认改为无链接编排夹具，核心语义有纯函数锚点。**仍需（2026-09-17 独立复核结论，本条此前口径有误）**：真实共享链接测试在创建夹具前**提前 `return`**，被记为 passed 而非 skipped，全量结果因此显示 `ignored=0`；该文件共 17 处此类守卫（16 处完全静默）。「在缺少链接权限的账号上零失败」不等于通过，而是**没有执行断言**。必须：(1) 在有符号链接能力的宿主或 macOS 上实跑并回填 RC-15；(2) 把静默 return 改为可观测跳过；(3) 恢复 `governance_batch_requires_the_shared_impact_confirmation_per_row` 被削弱的断言（RC-16）。
