...
### 9.1 cc-switch 参考实现验证

以本机 `C:\project\cc-switch` 的锁定提交作为供应商兼容信息参考源，提取以下信息：

- 产品线及默认 Base URL。
- API 格式、认证方式和端点拼接规则。
- 模型目录、思考参数及工具/Schema 专属转换。
- 已有的供应商回归测试和历史兼容分支。

cc-switch 证据不能单独证明 SkillHub 可用。其模型检查主要验证短文本调用和流式首字节，不覆盖 SkillHub 的结构化任务；源码中也存在“未逐一验证”的预设和旧配置兼容逻辑。因此，每个 SkillHub 内置预设采用四层证据：

1. **cc-switch 取证**：记录对应源码提交、文件和行号。
2. **官方文档核验**：复核当前产品线、端点、协议、模型和特殊参数；与 cc-switch 冲突时以官方当前契约为准，并记录差异。
3. **SkillHub 契约测试**：对请求规划结果、认证、响应提取和结构化策略进行 mock 验证。
4. **SkillHub 真实服务验证**：在获得用户授权凭据后分别执行普通模型调用和结构化输出调用。

兼容矩阵分别记录四层状态。cc-switch 默认配置、注释或测试通过均不能直接升级为 SkillHub“已支持”；只有满足 SkillHub 对应契约和真实服务标准后才升级支持级别。模型名称、API 版本和端点不得直接复制，必须先经过官方文档复核。
