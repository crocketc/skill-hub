...
test.describe("discovery shared skill cards", () => {
  test("online search renders shared cards with honest statuses and independent semantics", async ({ page }) => {
    ...
    // PageFrame 不再叠加第二层内边距（共享断言见 page-rhythm.ts）。
    await expectUnifiedPageRhythm(page, ".sh-preview-board__themes", "discovery");

    await expect.poll(() => rootHorizontalOverflow(page)).toBeLessThanOrEqual(0);
  });

  // E3：节奏断言不再只看单视口——800/1024/1280 全宽度锁定同一节奏。
...
test.describe("local discovery workbench agent groups (P1-06)", () => {
  test("renders one generic card for shared-reference targets with unavailable-last", async ({ page }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await page.goto(PREVIEW);

    await expect(page.getByRole("heading", { name: "Discovered agent directories" })).toBeVisible();
    // OPT-07：`.agents/skills` 的跨品牌 shared_reference 引用聚合为一张通用
    // 归属卡（Agent Skills / Shared directory），不产出 ZCode 品牌卡。
  ...
  test("exclusion runs after confirmation, removes only that card, and never deletes files", async ({ page }) => {
    ...
    // 其余卡片（含 `.agents/skills` 的通用共享归属卡）不受影响。
...
test.describe("discovery cards in Simplified Chinese", () => {
  ...
  test("renders Chinese card copy and statuses", async ({ page }) => {
    ...
    // OPT-07：`.agents/skills` 只有一张通用共享归属卡（共享目录 + 共享计数）。
    const sharedCard = page.getByTestId("agent-card-phys-agents");
    await expect(sharedCard).toHaveCount(1);
